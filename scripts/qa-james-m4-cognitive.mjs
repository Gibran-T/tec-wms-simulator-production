 ...
 *   JAMES_PASSWORD=*** BASE_URL=https://... node scripts/qa-james-m4-cognitive.mjs
 *
 * Cleanup:
 *   Leaves James READY FOR CLASS via admin.prepareDemoReadiness when
 *   TEC_ADMIN_EMAIL / TEC_ADMIN_PASSWORD (or teacher demo) are set.
 */
import {
  getM4CognitiveOption,
  listM4CognitiveQuestions,
} from "../shared/m4CognitiveSelectors.ts";
...
const report = {
  base: BASE,
  catalogOptions: listM4CognitiveQuestions().length,
  jamesLogin: null,
  scenarioId: null,
  runId: null,
  wrongAttempt: null,
...
try {
  // Sanity: local catalog present in this checkout
  if (report.catalogOptions !== 9) {
    report.errors.push(`Expected 9 cognitive questions, found ${report.catalogOptions}`);
  }
  ...
  const scn012 =
    list.find((s) => String(s?.code || s?.scnCode || "").includes("SCN-012")) ||
  ...
  if (!runId) throw new Error(`Failed to start run: ${JSON.stringify(start.data)?.slice(0, 300)}`);
  report.runId = runId;

  await trpcMutation("m4.submitKpiData", { runId });

  const wrong = getM4CognitiveOption("SCN-012", "KPI_ROTATION", "scn012-rot-a");
  const wrongRes = await trpcMutation("m4.submitKpiRotation", {
    runId,
    optionId: wrong.id,
  });
  report.wrongAttempt = {
    status: wrongRes.status,
    message: wrongRes.error?.json?.message || wrongRes.data?.error?.json?.message || null,
  ...
  const rotRes = await trpcMutation("m4.submitKpiRotation", { runId, optionId: rot.id });
  report.correctRotation = { status: rotRes.status, ok: rotRes.status < 400 };

  const svc = getM4CognitiveOption("SCN-012", "KPI_SERVICE", "scn012-svc-d");
  const svcRes = await trpcMutation("m4.submitKpiService", { runId, optionId: svc.id });
  report.correctService = { status: svcRes.status, ok: svcRes.status < 400 };

  const diag = getM4CognitiveOption("SCN-012", "KPI_DIAGNOSTIC", "scn012-diag-c");
  const diagRes = await trpcMutation("m4.submitKpiDiagnostic", { runId, optionId: diag.id });
  report.correctDiagnostic = { status: diagRes.status, ok: diagRes.status < 400 };

  const comp = await trpcMutation("m4.submitComplianceM4", { runId });
  report.compliance = { status: comp.status, ok: comp.status < 400 };

  const detail = await trpcQuery("runs.get", { runId });
  report.score = detail.body?.score ?? detail.body?.run?.score ?? null;

  // Cleanup → READY FOR CLASS
  if (ADMIN_EMAIL && ADMIN_PASSWORD) {
    ...
    report.cleanup = { skipped: true, reason: "no admin credentials" };
  }
