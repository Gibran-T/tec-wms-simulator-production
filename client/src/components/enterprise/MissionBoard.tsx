        ...
        >
          <div>
            <div className="flex flex-wrap items-center gap-2 mb-3">
              <span className="text-[10px] font-bold uppercase tracking-wider text-slate-600 bg-slate-100 dark:bg-slate-800 dark:text-slate-300 px-1.5 py-0.5">
                {t("Affectation — Mission", "Assignment — Mission")}
              </span>
              <PriorityBadge priority={assignmentMeta.priority} language={language} />
              <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500 bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5">
                {language === "FR"
            ...
            {scnCode && (
              <p className="text-[10px] font-mono text-slate-400 uppercase tracking-wider mb-3">{scnCode}</p>
            )}
            ...
            <div className="flex flex-wrap gap-2 mb-3">
              ...
              {completedRun && completedRun.score != null && (
                <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300">
                  <BarChart2 size={12} className="mr-1" /> {t("Résultat:", "Outcome:")} {completedRun.score}/100
                </span>
              )}
            </div>
          </div>
    ...
    if (groupBy === "department") {
      ...
      return groups.map((group) => (
        <div key={group.department} className="space-y-3">
          <h4 className="text-sm font-bold uppercase tracking-wider text-slate-500 border-b border-slate-200 dark:border-slate-700 pb-2">
            {language === "FR" ? group.label.fr : group.label.en}
          </h4>
    ...
    return groups.map((group) => (
      <div key={group.priority} className="space-y-3">
        <div className="flex items-center gap-2 border-b border-slate-200 dark:border-slate-700 pb-2">
          <PriorityBadge priority={group.priority} language={language} />
