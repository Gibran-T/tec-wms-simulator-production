...
describe("M4 score restoration — runtime budget", () => {
  ...
  it("perfect-run scoring events sum to 100", () => {
    ...
    const events = [
      ...
      { pointsDelta: scoreKpiInterpretation("rotationRate", "Rotation normale et equilibree", kpiResult).pointsDelta },
      { pointsDelta: scoreKpiInterpretation("serviceLevel", "Service excellent optimal", kpiResult).pointsDelta },
      { pointsDelta: scoreKpiInterpretation("diagnostic", diag, kpiResult, "SCN-012").pointsDelta },
  ...
  it("one incorrect rotation (−5) still passes at 75", () => {
    ...
    expect(calculateTotalScore(events as any)).toBe(75);
  });
});
