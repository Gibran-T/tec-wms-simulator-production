...
/** Canonical CC_RECON target key: runId + CC_RECON + sku + bin. */
export function buildCcReconTargetKey(runId: number, sku: string, bin: string): string {
  return `${runId}::CC_RECON::${sku}::${bin}`;
}
...
export function evaluateCycleCountReconProgress(
  ...
  for (const target of targets) {
    const { status, varianceQty } = getCcReconTargetStatus(target, counts, adjustments, transactions);
...
export function canExecuteStepM3(
  step,
  completedSteps,
  initialStateJson?: M3InitialStateJson | Record<string, unknown> | null,
) {
  ...
  if (!stepDef) {
    return {
      allowed: false,
      reason: "Step not part of this M3 scenario",
      reasonFr: "Cette étape ne fait pas partie de ce scénario M3",
      reasonEn: "This step is not part of this M3 scenario",
    };
  }
