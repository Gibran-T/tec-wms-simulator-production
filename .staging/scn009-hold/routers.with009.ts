...
export const appRouter = router({
  ...
  m3: router({
    ...
    /** M3 Step 3: CC_RECON — reconcile & adjust (idempotent per run/sku/bin target) */
    submitCcRecon: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (reconBefore.complete) {
          // Idempotent: all targets already reconciled — no new writes.
        ...
        for (const adj of input.adjustments) {
          ...
          if (!submissionCheck.allowed) {
            if (submissionCheck.reason?.includes("justification") || submissionCheck.reasonFr?.includes("justification")) {
              if (!run.isDemo) {
                await addScoringEvent({
                  runId: input.runId,
          ...
          // Re-check against latest persistence before write (concurrency / retry safety).
          const latestAdjustments = await getInventoryAdjustmentsByRun(input.runId);
          const latestState = await buildRunState(input.runId);
          const latestStatus = getCcReconTargetStatus(
            target,
            inventoryCounts,
            latestAdjustments,
          ...
          if (expectedVarianceQty === 0) {
            // Zero-variance: confirm without ADJ transaction or inventory mutation.
            const result = await upsertInventoryAdjustment({
              runId: input.runId,
              sku: adj.sku,
              varianceQty: 0,
              adjustmentQty: 0,
              reason: adj.justification.trim() || "ZERO_VARIANCE_CONFIRMED",
            });
            if (!result.created) idempotentHits += 1;
            continue;
          }

          const adjWrite = await upsertInventoryAdjustment({
            runId: input.runId,
            sku: adj.sku,
            varianceQty: expectedVarianceQty,
            adjustmentQty: expectedVarianceQty,
            reason: adj.justification.trim() || undefined,
          });
          const txWrite = await ensureCcReconAdjTransaction({
            runId: input.runId,
            sku: adj.sku,
            bin: adj.bin || target.bin || "",
            qty: expectedVarianceQty,
            comment: adj.justification.trim() || null,
          });
          if (adjWrite.created || txWrite.created) {
