...
/**
 * Idempotent inventory adjustment upsert for CC_RECON.
 * One logical row per (runId, sku): if a matching adjustmentQty already exists, no write.
 * Does not mutate historical duplicate rows (e.g. run 491) — only skips creating another.
 */
export async function upsertInventoryAdjustment(data: {
  runId: number; sku: string; varianceQty: number; adjustmentQty: number; reason?: string;
  ...
  const existing = await db
    ...
    .where(and(eq(inventoryAdjustments.runId, data.runId), eq(inventoryAdjustments.sku, data.sku)));
  if (existing.some((row) => Number(row.adjustmentQty) === data.adjustmentQty)) {
  ...
  await db.insert(inventoryAdjustments).values({
    ...
    sku: data.sku,
    varianceQty: String(data.varianceQty),
    adjustmentQty: String(data.adjustmentQty),
...
/**
 * Idempotent CC_RECON ADJ transaction insert.
 * Skips when a matching posted ADJ already exists for the same run/sku/bin/qty
 * or when the deterministic CC_RECON docRef is already present.
 */
export async function ensureCcReconAdjTransaction(data: {
  runId: number;
  sku: string;
  bin: string;
  qty: number;
  ...
  const db = await getDb();
  if (!db) throw new Error("DB not available");
  const docRef = `CC_RECON:${data.sku}:${data.bin}`;
  const existing = await db
    .select()
    .from(transactions)
    .where(and(eq(transactions.runId, data.runId), eq(transactions.docType, "ADJ")));
  const already = existing.some(
    (t) =>
      (t.sku === data.sku &&
        t.bin === data.bin &&
        t.posted &&
        Number(t.qty) === data.qty) ||
      t.docRef === docRef,
  );
