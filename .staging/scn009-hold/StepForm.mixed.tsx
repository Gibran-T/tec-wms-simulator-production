...
export default function StepForm() {
  ...
  const isAdjStepEarly = baseCfg.code === "ADJ" || step?.toLowerCase() === "adj";
  ...
  const isAdjStep = isAdjStepEarly;
  ...
  const ccReconProgress = useMemo(() => {
    ...
    const evidence = (runData as { m3Evidence?: {
      inventoryCounts?: Array<{ sku: string; systemQty: number | string; countedQty: number | string; varianceQty?: number | string }>;
      inventoryAdjustments?: Array<{ sku: string; varianceQty: number | string; adjustmentQty: number | string }>;
    } } | undefined)?.m3Evidence;
    const txs = ((runData as { transactions?: Array<{ docType: string; sku: string; bin?: string; qty: number; posted?: boolean }> } | undefined)?.transactions) ?? [];
        ...
        {(isCurrentStep || (isDemo && !isCompleted)) && !isLocked && (
          <div className="bg-card border border-border border-t-0 rounded-b-md">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="p-5 space-y-4">
              ...
              {/* CC confirmatory banner removed for replenishment-only scenarios (effective steps exclude CC). */}
              ...
              {cfg.fields.includes("sku") && !(step?.toLowerCase() === "cc_recon" && ccReconProgress && ccReconProgress.pendingSkus.length === 0) && (
                <div>
                  <label className="fiori-field-label">
              ...
              {step?.toLowerCase() === "cc_recon" && (
                <div className="mb-4 space-y-3">
                  ...
                  {ccReconProgress && ccReconProgress.pendingSkus.length === 0 && ccReconProgress.requiredCount > 0 && (
              ...
              {/* Variance Qty field */}
              {cfg.fields.includes("varianceQty") && !(step?.toLowerCase() === "cc_recon" && ccReconProgress && ccReconProgress.pendingSkus.length === 0) && (
                <div>
                  <label className="fiori-field-label">
                    {selectedCcReconVariance === 0
              ...
              {/* Justification field */}
              {cfg.fields.includes("justification") && !(step?.toLowerCase() === "cc_recon" && ccReconProgress && ccReconProgress.pendingSkus.length === 0) && (
                <div>
                  <label className="fiori-field-label">
                    {selectedCcReconVariance === 0
