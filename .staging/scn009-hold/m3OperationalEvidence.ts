...
export function computeCcReconProgress(
  ...
  statuses: Array<{ sku: string; bin?: string; status: M3ReconTargetStatus; varianceQty: number | null; systemQty?: number; physicalQty?: number }>;
} {
...
export function getM3GridStatus(
  ...
  if (scnCode === "SCN-009" || scnCode === "SCN-010") {
    ...
    if (status === "RECONCILED_WITH_ADJUSTMENT") return "RECONCILED_WITH_ADJUSTMENT";
    if (status === "RECONCILED_NO_ADJUSTMENT") return "RECONCILED_NO_ADJUSTMENT";
    if (hasOpenVariance(sku, inventoryCounts, inventoryAdjustments, transactions)) return "VARIANCE_OPEN";
    if (varianceQty != null && varianceQty !== 0 && status === "PENDING") return "VARIANCE_OPEN";
  }
